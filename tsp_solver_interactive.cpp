...
void run_case(int n, int seed, const string& kind, int hk_limit = 10) {
 ...
 int main() {
    ...
    string kind;
    int seed = 2; 
    int HK_LIMIT = 10; // It is safely right here!

    // 1. Get the number of nodes
    cout << "\n=== TSP Stress Tester ===" << endl;
    cout << "Enter the number of nodes (n = ?): ";
    cin >> n;

    // 2. Get the matrix kind
    cout << "\nSelect matrix kind:" << endl;
    cout << "  1. Blocked" << endl;
    cout << "  2. Symmetric" << endl;
    cout << "  3. Asymmetric" << endl;
    cout << "Enter 1, 2, or 3: ";
    cin >> kind_choice;

    // 3. Map the digital input to the correct string
    if (kind_choice == 1) {
        kind = "blocked";
        ...
        cout << "Invalid choice. Defaulting to asymmetric." << endl;
        kind = "asymmetric";
    }

    // 4. Run the requested case
    run_case(n, seed, kind, HK_LIMIT);
